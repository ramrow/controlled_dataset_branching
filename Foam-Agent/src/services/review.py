...
class FeasibilityPydantic(BaseModel):
    unsupported_openfoam10: bool = Field(description="True if user requirement is fundamentally beyond OpenFOAM10 capabilities in this workflow")
    reason: str = Field(description="Short reason for infeasibility decision")
...
def detect_openfoam10_infeasible(
    user_requirement: str,
    error_logs: List[str],
    review_analysis: str,
) -> dict:
    """Detect fundamentally unsupported requirements to stop futile reviewer loops."""
    system_prompt = (
        "You are an OpenFOAM10 feasibility checker. Decide if this task is fundamentally unsupported "
        "for OpenFOAM10 in a standard Foam-Agent workflow, even after file corrections. "
        "Examples: requesting non-existent OpenFOAM features/solvers, impossible coupled physics not available in OF10, "
        "or requirements that contradict solver capabilities by design. "
        "Do NOT mark unsupported for normal syntax/configuration mistakes; those are fixable. "
    ...
    user_prompt = (
        ...
        "Return JSON with keys: unsupported_openfoam10 (bool), reason (string)."
    )

    resp = global_llm_service.invoke(user_prompt, system_prompt, pydantic_obj=FeasibilityPydantic)
